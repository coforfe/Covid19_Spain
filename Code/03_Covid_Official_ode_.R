...
tic()
rm(list = ls())
options(scipen = 999) # To prevent scientific notation in ggplot

# Get Daily file
url_data <- c('https://covid19.isciii.es/resources/serie_historica_acumulados.csv')
cof_val  <- c('Carlos Ortega - cof@qualityexcellence.es')

datRaw <- readLines(con <- file(url_data))
close(con)
datRaw <- datRaw[1:(length(datRaw) - 3)]# Han incluido Prevalencias...
head(datRaw)
tail(datRaw)

com_val <- data.frame(
  acro = c("AN","AR","AS","IB","CN","CB","CM","CL","CT","CE","VC","EX","GA","MD","ML","MC","NC","PV","RI"),
  name_ccaa = c('Andalucía', 'Aragón', 'Asturias', 'Islas Baleares', 'Canarias', 'Cantrabria', 'Castilla Mancha',
           'Castilla León', 'Cantabria', 'Ceuta', 'Comunidad Valencia', 'Extremadura','Galicia', 'Madrid',
...
# Read Saved file
datInTmp <- fread(my_title, fill = TRUE)
names(datInTmp)[1] <- c('CCAA')
datInTmp[ , fecha := dmy(FECHA)]
datInTmp[, FECHA := NULL]
setnames(datInTmp, 'CASOS', 'Casos')

datIn <- merge(datInTmp, com_val, by.x = c('CCAA'), by.y = c('acro'), all.x = TRUE)



#-------- Create New Columns --------------------------------

# https://blog.ephorie.de/covid-19-in-the-us-back-of-the-envelope-calculation-of-actual-infections-and-future-deaths#more-3729
my_shift <- function(x, n = 10) { c(x[-(seq(n))], rep(NA, n)) }
datIn[ , InfReal   := my_shift( round( Fallecidos / 0.007 )) , by = .(CCAA)]
datIn[ , perReal   := round( Casos / InfReal , 4) , by = .(CCAA)]

#Lag
my_cols <- c('Casos', 'Hospitalizados', 'UCI', 'Fallecidos', 'Recuperados', 'InfReal')
...
datIn[ , dif_Real  := InfReal - lag_InfReal, by = .(CCAA)]
datIn[ , per_Real  := round(100*dif_Real/InfReal, 2) , by = .(CCAA)]
...
datIn[ , tot_Infec := sum(InfReal), by = .(fecha)]
datTot <- unique(datIn[ , .(tot_Casos, tot_Falle, tot_UCI, tot_Hospi, tot_Recu, tot_Infec), by = .(fecha)])

my_colstot <- c('tot_Casos', 'tot_Hospi', 'tot_UCI', 'tot_Falle', 'tot_Recu', 'tot_Infec')
...
datTot[ , dif_Infec := tot_Infec - lagtot_Infec]
datTot[ , per_Infec := round(100*dif_Infec/tot_Infec, 2) ]


#----------------------  Functions for Charting ----------------------
# ------- Lag
var_ori <- c('Casos', 'Hospitalizados', 'UCI', 'Fallecidos','Recuperados','InfReal')
var_per <- c('per_Casos', 'per_Hospi', 'per_UCI', 'per_Falle', 'per_Recu', 'per_Real')
var_dif <- c('dif_Casos', 'dif_Hospi', 'dif_UCI', 'dif_Falle', 'dif_Recu', 'dif_Real')
...
f_gr <- function(df, colum, my_tit) {
  gg <- ggplot(df,  aes(x  = fecha,  y  = .data[[colum]]) ) + 
    ...
    facet_wrap( ~ name_ccaa, scales = 'free') +
    geom_rect(data = subset(df, name_ccaa %in% c('Madrid') ), 
              fill = "paleturquoise", xmin = -Inf, xmax = Inf,
              ymin = -Inf, ymax = Inf, width = 2) + 
    geom_line( colour = 'grey', alpha = 0.25) +
    geom_point(size = 0.5) +
    ggtitle( my_tit, subtitle = url_data ) +
    labs( caption = cof_val) +
...
for (i_var in var_dif) {
  ...
  #--- Madrid
  gr_lag_mad <- f_gr_one(datIn[ CCAA == 'MD',], i_var, my_tit)
  print(gr_lag_mad)
...
#------------  Casos vs. Fallecidos
datCaFa <- datIn[ , .(fecha, Casos, Fallecidos, Recuperados, CCAA, name_ccaa)]
datCaFa_lg <- melt(datCaFa, id.vars = c('fecha', 'CCAA', 'name_ccaa'), measure.vars = c('Casos', 'Fallecidos', 'Recuperados'))

#----- Todas CCAA
ggplot( datCaFa_lg, aes( x = fecha, y = value, group = variable, fill = variable)) +
  # geom_point( ) +
  facet_wrap( ~ name_ccaa, scales = 'free') +
  geom_rect(data = subset(datCaFa_lg, name_ccaa %in% c('Madrid') ), 
            fill = "paleturquoise" , colour = 'red' , xmin = -Inf,xmax = Inf,
            ymin = -Inf,ymax = Inf, width = 12)  + 
...
datHoFaCa <- datIn[ , .(fecha, Hospitalizados, Fallecidos, Casos, Recuperados, CCAA, name_ccaa)]
datHoFaCa_lg <- melt(datHoFaCa, id.vars = c('fecha', 'CCAA', 'name_ccaa'), measure.vars = c('Hospitalizados', 'Fallecidos', 'Casos', 'Recuperados'))

#----- Todas CCAA
ggplot( datHoFaCa_lg, aes( x = fecha, y = value, group = variable, fill = variable)) +
  # geom_point( ) +
  facet_wrap( ~ name_ccaa, scales = 'free') +
  geom_rect(data = subset(datHoFaCa_lg, name_ccaa %in% c('Madrid') ), 
            fill = "paleturquoise", colour = 'red', xmin = -Inf,xmax = Inf,
            ymin = -Inf,ymax = Inf, width = 2 ) + 
  geom_line( aes(colour = variable), alpha = 0.25) +
  geom_point(aes(colour = variable), size = 0.3) +
